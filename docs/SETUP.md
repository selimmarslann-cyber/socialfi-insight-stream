...
- [ ] **ENV parity:** Production (Netlify) ve Staging (Loveable) ortamlarında `NEXT_PUBLIC_SUPABASE_URL` ve `NEXT_PUBLIC_SUPABASE_ANON_KEY` anahtarlarını mutlaka tanımlayın. Vite tabanlı dağıtımlarda aynı değerleri `VITE_SUPABASE_URL` ve `VITE_SUPABASE_ANON_KEY` olarak da ekleyin.
