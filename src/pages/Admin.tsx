...
import { fetchAdminMetrics } from "@/lib/adminAnalytics";
  ...
  return (
    <>
      <DashboardCard className="space-y-3">
        <DashboardSectionTitle label="Overview" title="Ops / Analytics Dashboard" />
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <StatCard
            label="Total Users"
            value={metrics ? metrics.totalUsers.toString() : overview ? overview.profiles.toString() : "—"}
            icon={Users}
          />
          <StatCard
            label="Total Posts"
            value={metrics ? metrics.totalPosts.toString() : overview ? overview.posts.toString() : "—"}
            icon={Shield}
          />
          <StatCard
            label="Total Positions"
            value={metrics ? metrics.totalPositions.toString() : overview ? overview.pools.toString() : "—"}
            icon={TrendingUp}
          />
          <StatCard
      ...
      {metrics && metrics.alphaDistribution.length > 0 ? (
        <DashboardCard className="space-y-3">
          <DashboardSectionTitle label="Alpha Distribution" title="Score Distribution" />
          <div className="space-y-2">
            {metrics.alphaDistribution.map((bucket) => {
              const maxCount = Math.max(...metrics.alphaDistribution.map((b) => b.count), 1);
              const widthPercent = maxCount > 0 ? (bucket.count / maxCount) * 100 : 0;
              return (
                <div key={bucket.bucket} className="space-y-1">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-semibold text-text-primary">{bucket.bucket}</span>
                    <span className="text-text-secondary">{bucket.count} users</span>
                  </div>
      ...
      {metrics && metrics.topAlphaUsers.length > 0 ? (
        <DashboardCard className="space-y-3">
          <DashboardSectionTitle label="Alpha Leaderboard" title="Top Alpha Users" />
          <div className="space-y-2">
            {metrics.topAlphaUsers.map((user, index) => (
              <div
              ...
              >
                <div>
                  <p className="text-sm font-semibold text-text-primary">
                    #{index + 1} {user.wallet_address.slice(0, 6)}…{user.wallet_address.slice(-4)}
                  </p>
                  <p className="text-[11px] text-text-muted">
                    {user.total_positions} positions
                  </p>
                </div>
                <span className="text-lg font-semibold text-indigo-400">
                  {Math.round(user.alpha_score)}
                </span>
              </div>
            ))}
          </div>
...
const fetchAdminUsers = async () => {
  ...
  const { data, error } = await supabase
    ...
    .select("id, display_name, wallet_address, handle, nop_id, is_banned, is_verified, created_at, total_posts")
...
const AdminUsersTab = () => {
  ...
  const toggleVerify = useMutation({
    mutationFn: async ({ id, next }: { id: string; next: boolean }) => {
      if (!supabase) throw new Error("Supabase not configured");
      const { error } = await supabase.from("social_profiles").update({ is_verified: next }).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["admin-users"] });
    ...
    onError: () => toast.error("Unable to update verification status."),
  });
                  ...
                  <TableRow key={user.id}>
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-2">
                        ...
                        {user.is_verified ? (
                          <Badge variant="default" className="rounded-full bg-cyan-500/20 text-cyan-400">
                            Verified
                          </Badge>
                        ) : null}
                      </div>
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-2">
                        ...
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => toggleVerify.mutate({ id: user.id, next: !user.is_verified })}
                        >
