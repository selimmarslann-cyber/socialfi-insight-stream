...
function Rank({ i }: { i: number }) {
  const map = ["🥇", "🥈", "🥉"];
  return (
    <div className="w-7 h-7 grid place-items-center rounded-full border bg-white text-[15px] shadow-sm">
      {map[i] || `#${i + 1}`}
    </div>
  );
}
      ...
      <div className="space-y-2">
        {rows === null &&
          Array.from({ length: 5 }).map((_, i) => (
            <div
              key={i}
              className="p-2.5 rounded-lg border bg-white/70 animate-pulse h-[64px]"
            />
          ))}
        {Array.isArray(rows) &&
          rows.map((r: any, i: number) => (
            <div
              key={r.user_id}
              className="flex items-center justify-between p-2.5 rounded-lg border bg-white/70"
            >
              <div className="flex items-center gap-3">
                <Rank i={i} />
                <Avatar name={r.profiles?.username || null} id={r.user_id} />
                <div className="flex flex-col">
                  <div className="font-medium text-[#0F172A] leading-tight">
                    @{r.profiles?.username || shortId(r.user_id)}
                  </div>
                  <div className="text-[12px] text-[#475569] leading-tight opacity-80">
                    NOP{r.user_id.slice(0, 5)}
                  </div>
                </div>
              </div>
        ...
        {Array.isArray(rows) && rows.length === 0 && (
          <div className="text-[13px] text-[#475569]">No users yet.</div>
        )}
      </div>
    </div>
  );
