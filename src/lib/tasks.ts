...
export async function detectCompletion(task: TaskKey, userId?: string) {
  ...
  if (task === 'deposit') {
    const { data, error } = await supabase.from('profiles').select('wallet_address').eq('id', uid).single()
    if (error) throw new Error(error.message)
...
export async function claimTask(task: TaskKey) {
  ...
  if (!rec || !rec.completed_at) throw new Error('Görev şartı henüz sağlanmadı.')
  if (rec.claimed_at) throw new Error('Bu ödül daha önce alındı.')

  const reward = Number(rec.reward_nop || 0)
  ...
  const updatedPoints = current + reward
